            ...
            if let Some(player) = p {
                if let Some(skater_obj_index) = player.skater {
                    if let HQMGameObject::Player(skater) = & mut self.game.objects[skater_obj_index] {
                        ...
                        match player.team{
                            HQMTeam::Red=>{
                                skater.set_orientation(Point3::new(15.0+(self.roles[player.role_index].faceoff_offsets[faceoff_position_index].x),self.roles[player.role_index].faceoff_offsets[faceoff_position_index].y,30.5+(self.roles[player.role_index].faceoff_offsets[faceoff_position_index].z)), Rotation3::from_euler_angles(0.0,0.0,0.0));
                            },
                            HQMTeam::Blue=>{
                                skater.set_orientation(Point3::new(15.0+(self.roles[player.role_index].faceoff_offsets[faceoff_position_index].x*-1.0),self.roles[player.role_index].faceoff_offsets[faceoff_position_index].y,30.5+(self.roles[player.role_index].faceoff_offsets[faceoff_position_index].z*-1.0)), Rotation3::from_euler_angles(0.0,std::f32::consts::PI,0.0));
                            },
                            _=>{
