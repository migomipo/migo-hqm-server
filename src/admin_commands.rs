...
impl HQMServer {
    ...
    pub(crate) fn mute_player(
        ...
        if let Some(admin_player) = self.state.players.get_player(admin_player_id) {
            if admin_player.is_admin {
                ...
                self.state.admin_deny_message(admin_player_id);
            }
        }
    }
    ...
    pub(crate) fn mute_chat(&mut self, player_id: PlayerId) {
        if let Some(player) = self.state.players.get_player(player_id) {
            if player.is_admin {
                self.is_muted = true;

                let msg = format!("Chat muted by {}", player.player_name);
                info!("{} ({}) muted chat", player.player_name, player_id);
                self.state.add_server_chat_message(msg);
            } else {
                self.state.admin_deny_message(player_id);
            }
    ...
    pub(crate) fn unmute_chat(&mut self, player_id: PlayerId) {
        if let Some(player) = self.state.players.get_player(player_id) {
            if player.is_admin {
                self.is_muted = false;

                let msg = format!("Chat unmuted by {}", player.player_name);
                info!("{} ({}) unmuted chat", player.player_name, player_id);
    ...
    pub(crate) fn restart_server(&mut self, player_id: PlayerId) {
        if let Some(player) = self.state.players.get_player(player_id) {
            if let Some(server_service) = self.config.server_service.as_deref() {
                if player.is_admin {
                    ...
                    if let Err(_) = restart(server_service) {
                        self.state
                            .add_directed_server_chat_message("Restart failed", player_id);
                    }
                } else {
                    self.state.admin_deny_message(player_id);
                }
    ...
    pub(crate) fn kick_all_matching<B: GameMode>(
        ...
        if let Some(player) = self.state.players.get_player(admin_player_id) {
            if player.is_admin {
        ...
        if let Some(player) = self.state.players.get_player(admin_player_id) {
            if player.is_admin {
                ...
                if kick_player_index != admin_player_id.index {
                    if let Some((kick_player_id, kick_player)) =
                        self.state.players.get_player_by_index(kick_player_index)
                    {
    ...
    pub(crate) fn clear_bans(&mut self, player_id: PlayerId) {
        if let Some(player) = self.state.players.get_player(player_id) {
            if player.is_admin {
                self.ban.clear();
                info!("{} ({}) cleared bans", player.player_name, player_id);
    ...
    pub fn set_replay(&mut self, player_id: PlayerId, rule: &str) {
        if let Some(player) = self.state.players.get_player(player_id) {
            if player.is_admin {
                match rule {
                    "on" => {
                        ...
                        info!("{} ({}) enabled replays", player.player_name, player_id);
